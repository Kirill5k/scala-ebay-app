...
private[mappers] object GameDetailsMapper {
  ...
  private val PRIMARY_TITLE_WORDS_REPLACEMENTS = List(
    ...
    "(complete|standard|std|classic(s)?|(\\d+(th)?)? anniversary|remastered|elite|\\beu\\b|coll(ector(s)?)?|ltd|goty|(action )?game of the|legacy( pro)?|(un)?limited|premium|(digital )?deluxe|standard|ultimat)(?s).* (collection|edition|\\be(d)?(i)?(t)?(i)?\\b)(?s).*$",
    ...
    "Microsoft", "sony", "electronic arts", "nintendo", "square enix", "ea sport(s)?", "Dreamworks",
    "[^\\p{L}\\p{N}\\p{P}\\p{Z}]",
    "\\d{6,}"
  ).mkString("(?i)", "|", "")

  private val SECONDARY_TITLE_WORDS_REPLACEMENTS = List(
    "(the )?((action|official|console|gold|kids) )?(video( )?)?game(s)?( (console|of the year|for( the)?))?",
    "(complete )?(brand|game) (new|neuf|nuevo)", "\\bpegi( \\d+)?\\b(?s).*$", "\\d+th anniversary", "disc (mint|vgc)", "platinum",
    ...
    "(\\bUK\\b|\\bEU\\b|genuine|european)(( |-)(new|only|seller|version|stock|import))?", "For age(s)? \\d+(\\+)?", "complete", "must see",
    ...
    "\\bMarvels\\b", "^\\bMARVEL\\b", "^SALE", "NEW$", "^BOXED", "^SALE", "^(NEW\\s+)?(and )?SEALED", "^NEW", "^best", "^software", "very rare", "rare$", "bargain$", "mint$"
  ).mkString("(?i)", "|", "")

...
private[mappers] object GameDetailsMapper {
  ...
  private val LEVEL2_TITLE_WORDS_REPLACEMENTS = List(
    "((new|rare) )?((very )?good )?(\\b(for|((only|playable|plays) )?on)\\b )?(sony |microsoft )?(play( )?st(a)?(t)?(i)?(o)?(n)?(( )?\\d)?|x( )?box(( )?(one|\\d+))?|\\bps\\d\\b|\\bxb( )?(o(ne)?|\\d+)?\\b|(nintendo )?(switch|\\bwii( u)?\\b))( (edition|version))?(\\s+new)?( 20\\d\\d)?",
    "for (the )?playstation(\\s+)?vr", "((ps( )?)?(vr|move)|kinect) (required|compatible)",
    "(dbl|double|triple|twin|expansion|combo)( )?(pack|pk)",
    "(1st|2nd|first) class.*$", "(boxed|complete) (with|case)(?s).*$", "exclusive to(?s).*$", "((comes )?with|in original|no|missing|plus|inc(ludes|luding)?) (booklet|original|instructions|box|map|case|manual)(?s).*$", "(the )?disc(s)? (are|is|in)(?s).*$",
    "(new )?(((very|super) )?rare|limited run|(\\d+ )?new|pal|physical|great|boxed|full|complete|boxed( and)?\\s+complete) game(s)?( \\d+)?( new)?",
    "(in )?(great|(very )?good|incredible|ex(cellent)?|amazing|mint|superb|working|perfect|used|(fully )?tested|immaculate|fantastic) (dis(c|k)?|working|good|(working )?order|cond(ition)?|value|prices)",
    ...
    "Microsoft( 20\\d\\d)?", "sony", "electronic arts", "nintendo", "square enix", "Dreamworks", "Disney Pixar(s)?", "WB Games", "Bend Studio", "LucasArt(s)?",
    "[^\\p{L}\\p{N}\\p{P}\\p{Z}]",
    "\\d{6,}(\\w+)?"
  ).mkString("(?i)", "|", "")
  ...
  private val LEVEL3_TITLE_WORDS_REPLACEMENTS = List(
    ...
    "\\bpegi( \\d+)?\\b(?s).*$", "(\\d+th|(20|ten) year) (anniversary|celebration)", "(\\d|both)?( )?(disc(s)?|cd(s)?)( mint)?", "platinum", "brand new", "\\bID\\d+\\w", "18\\s+years",
    ...
    "remaster(ed)?", "directors cut", "\\bctr\\b", "original", "english", "deluxe", "standard", "\\bgoty\\b", "mult(i)?(-| )?lang(uage)?( in game)?", "freepost",
    "blu-ray", "bonus level", "Console Exclusive", "playable on", "Definitive Experience", "Highly Rated", "official$", "essentials", "classic(s)?", "boxed( and)? (complete|manual)",
    "For age(s)? \\d+(\\+)?", "must see", "see pics", "Best Seller(s)?",
    ...
    "\\bDVD\\b", "\\bAND\\b", "\\bNTSC\\b", "\\bWi1\\b", "\\bMarvels\\b", "\\bENG\\b", "\\bVGWO\\b",
    "\\bMarvels\\b", "SEALED$", "NEW$", "very rare",
  ...
  private def sanitizeTitle(title: String): Option[String] =
    title
      ...
      .replaceFirst("(?i)(?<=.{10})(the )?\\w+(?=\\s+(\\be(d)?(i)?(t)?(i)?(o)?(n)?\\b|coll(ection)?)) (\\be(d)?(i)?(t)?(i)?(o)?(n)?\\b|coll(ection)?)(?s).*$", "")
      .replaceAll("é", "e")

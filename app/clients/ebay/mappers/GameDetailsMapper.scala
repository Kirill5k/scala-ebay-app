...
private[mappers] object GameDetailsMapper {
  ...
  private val TITLE_WORDS_FILTER = List(
    ...
    "\\bRPG\\b( game)?", "role( )?playng( game)?", "Adventure Role( playing)?", "Strategy (combat|Management)", "tactical shooter", "action (adventure|fighting)( game)?",
    ...
    "^SALE", "NEW$", "^SEALED", "^NEW", "\\bMarvels\\b", "^best", "^software", "very rare",
    "[^\\p{L}\\p{N}\\p{P}\\p{Z}]",
    "\\d{6,}"
  ).mkString("(?i)", "|", "")

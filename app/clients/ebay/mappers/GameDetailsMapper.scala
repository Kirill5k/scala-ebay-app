...
private[mappers] object GameDetailsMapper {
  ...
  private val LEVEL2_TITLE_WORDS_REPLACEMENTS = List(
    ...
    "((ps( )?)?(vr|move)|kinect) (needed|required|compatible)",
    ...
    "ubisoft",
    "currys",
    "Take (Two|2)( Interactive)?",
    "(EA|2k) (dice|music|sport(s)?|games)",
    "James Camerons",
    "\\bTom clancy(s)?\\b",
    "gamecube",
    "Bethesda(s)?( Softworks)?",
    "Hideo Kojima",
    "(bandai )?namco",
    "rockstar games",
    "James Bond",
  ...
  private val LEVEL3_TITLE_WORDS_REPLACEMENTS = List(
    ...
    "(the )?(official )?Strategy Combat( guide)?",
    "(First Person|FPS) Shooter",
    "(american|soccer) football( 20\\d\\d)?",
    "(auto|golf) sports",
    ...
    "football soccer",
    "((family fun|survival) )?Action Adventure( Open World)?",
    "(adventure )?survival horror",
    "fighting multiplayer",
    "Multi Player",
    "life simulation",
    "\\bpegi( \\d+)?\\b(?s).*$",
    "(\\d+th|(20|ten) year) (anniversary|celebration)",
    "(\\d|both)?( )?(dis(c|k)(s)?|cd(s)?)( (version|set|mint))?",
    "platinum",
    ...
    "limited run( \\d+)?",
    "box( )?set",
    "pre( )?(release|owned|enjoyed|loved)",
    "compatible",
    "physical copy",
    "(big|steel)( )?box( version)?",
    "no scratches",
    "(manual|instructions) included",
    "100 ebayer",
    "(condition )?very good",
    "reorderable",
    "(posted|sent|dispatched) same day",
    "in stock( now)?",
    "(only )?played once",
    "best price",
    "Special Reserve",
    "Expertly Refurbished Product",
    "(quality|value) guaranteed",
    "(trusted|eBay|best|from ebays biggest) Seller(s)?",
    ...
    "standard",
    "\\bgoty\\b",
    "mult(i)?( )?lang(uage)?(s)?( in game)?",
    "(with )?(fast|free)( )?(dispatch|post)",
    "fast free",
    "blu( )?ray",
    "Console Exclusive",
    "playable on",
    ...
    "Highly Rated",
    "essentials",
    "classic(s)?( (hit(s)?|version))?",
    "boxed(?s).*(complete|manual)",
    "very rare",
    "award winning",
    "official licenced",
    "Unwanted Gift",
    ...
    "\\bGC\\b",
    "\\bCIB\\b",
    "(100 )?((all|fully) )?complete( (instructions|package))?",
    "SEALED$",
    "NEW$"
  ).mkString("(?i)", "|", "")
  ...
  private val BUNDLE_MATCH_REGEX = List(
    "(new|multiple|PS4|PS3|xbox one|switch|wii( u)?) games",
    "bundle",
    "job(\\s+)?lot"
  ).mkString("(?i)", "|", "").r

...
class VideoGameEbayClient @Inject()(val ebayAuthClient: EbayAuthClient, val ebayBrowseClient: EbayBrowseClient) extends EbaySearchClient[GameDetails] {
  ...
  private val LISTING_NAME_TRIGGER_WORDS = List(
    ...
    "(canvas|replacement|cover|carry|travel(er)?|carrying|just( the)?|no|hard|storage|game|vault|phone|card|foreign|metal|protection|protective)\\s+(pouch|case|bag)",
    ...
    "headset", "\\bhdmi\\b", "\\bUSB\\b", "\\bhdd\\b", "(nintendo|switch) labo", "(steering|racing) wheel", "wristband", "horipad", "(cord|power|hdmi)( )?(adepter|level|supply)", "tv tuner",

...
class VideoGameEbayClient @Inject()(val ebayAuthClient: EbayAuthClient, val ebayBrowseClient: EbayBrowseClient) extends EbaySearchClient[GameDetails] {
  ...
  private val LISTING_NAME_TRIGGER_WORDS = List(
    ...
    "(cover|carry|travel|carrying|just the|no|hard|storage|game|vault|phone) (pouch|case|bag)",
    ...
    "arabic", "(no|protective|case|silicone|phone) cover(s)?",
    ...
    "family membership", "12 month", "dynamic theme", "account", "game(\\s+)?pass", "level boosting",

...
class VideoGameEbayClient @Inject()(val ebayAuthClient: EbayAuthClient, val ebayBrowseClient: EbayBrowseClient) extends EbaySearchClient[GameDetails] {
  ...
  private val LISTING_NAME_TRIGGER_WORDS = List(
    ...
    "family membership", "12 month", "dynamic theme", "themes", "account", "level boosting", "gamer score",
    "samsung", "huawei", "iphone",
    "\\bhori\\b", "\\bDE\\b",
    "ark survival(\\s+(\\w+|\\d+)){5,}",
    "diablo 3(\\s+(\\w+|\\d+)){6,}", "fortnite",
    "animal crossing (?s).* (recipe card|fossil|dinos)", "gta (?s).* (online|million)",
    "fallout 76(\\s+(\\w+|\\d+)){5,}", "((\\w+|\\d+)\\s+){3,}fallout 76", "fallout (?s).* (power|cap|armo)",
    "fifa 20(\\s+(\\w+|\\d+)){5,}", "fifa (?s).* point",
